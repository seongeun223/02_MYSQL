...
select menu_code        메뉴코드,
       ...
       orderable_status 주문가능여부
from tbl_menu
where category_code = ( -- 괄호 안에 쿼리문을 넣어서 서브쿼리로 사용 가능하다.
    select
        category_code
    from
        tbl_menu
...
-- 서브쿼리의 WHERE 절 수행을 위해, 메인쿼리가 언제 수행되든 구조
...
- FROM 절에서만 사용이 된다. (JOIN일 시 JOIN 구문에서도 가능)
*/
