...
-- concat_ws(구분자, 문자열1, 문자열2, ...) : 문자열을 구분자랑 같이 이어 붙임
...
-- insert : 기준 문자열의 위치부터 길이만큼 지우고 문자열을 끼워 넣는다.
...
-- SUBSTRING(문자열, 시작 위치, 길이) : 시작 위치부터 길이만큼의 문자를 반환(길이를 생략하면 시작부터 끝까지 반환)
select substring('안녕하세요 반갑습니다.', 7, 2),
       substring('안녕하세요 반갑습니다.', 7);

-- SUBSTRING_INDEX(문자열, 구분자, 횟수) : 구분자가 왼쪽부터 횟수 번째 나오면 그 이후의 오른쪽은 버린다.
-- 횟 수가 음수일 경우, 오른쪽부터 세고 왼쪽을 버린다.
select substring_index('hong.test@gmail.com', '.', 2);
select substring_index('hong.test@gamil.com', '.', -2);

-- 숫자형

-- FORMAT(숫자, 소수점 자리 수)
-- FORMAT : 1000단위마다 콤마(,) 표시해주며, 소수점 아래 자리 수(반올림)까지 표현한다.
