...
-- 둘 중에 하나라도 false면 false, 둘 다 true여야지 true
/*
true = 1; // false = 0
null : 값이 없다.
*/
...
주문가능여부가 Y이고, 가격이 10000원 이하인 데이터 조회
*/
select menu_name,
       menu_price,
       category_code,
...
-- 0 AND 0이 먼저 진행되므로 두 번째 연산은 1 or 0이 된다. => 1
select 1 or 0 and 0;
-- 1 or 0이 먼저 연산되므로 두 번째 연산 1 and 0 => 0
select (1 or 0) and 0;
...
- _ : 1개의 문자를 나타낸다.
ex) 'a_k'는 'a'로 시작하고 'k'로 끝나는 세 글자 문자열을 의미한다.
*/
select *
from tbl_menu
...
select menu_name,
       category_code
from tbl_menu
where category_code in (4, 5, 6) -- category_code가 (4, 5, 6) 목록 중에 있는 데이터만 조회해줘
